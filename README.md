...
| **Automobile Manufacturing Facilities** | Hand-researched defense-conversion dataset, generated by [`build_high_yield_facilities.py`](build_high_yield_facilities.py) (`auto_facilities_high_yield_defense.json`) | 72 facilities with genuine per-facility research (production processes, workforce, prior defense work). Scope-up beyond 72 via the bulk EPA-ECHO-style file (`auto_facilities_VECA8.json`, 6,935 records) is deliberately disabled (`INCLUDE_NAICS_DERIVED = False`) — that file's NAICS classification doesn't correlate with facility size or legitimacy (a Family Dollar retail store is tagged `facility_type: "Assembly Plant"` under NAICS 336111; `oem_or_parent` is just a copy of `facility_name`, not real parent-company data), so it can't be filtered down to "high-yield" automatically. Would need either a real employment/sqft field (absent from every record) or a manually curated allow-list of recognized OEM/Tier-1 names before it's safe to include. |
...
by fuel type (11 buckets); Automobile Manufacturing Facilities by defense conversion
role (5 categories: OEM Assembly, Engine & Powertrain, Metal Stamping & Body,
Battery & Energy Storage, Other Major Component); NERC Subregions by region (8 top-
...
flowchart TD
    ...
    subgraph PIPE["build_high_yield_facilities.py (reproducible, run manually)"]
        ...
        FILTER["INCLUDE_NAICS_DERIVED = False:\nEPA_MASTER path currently skipped"]
        SCOPED_JSON["auto_facilities_high_yield_defense.json\n(currently 72 — see table above)"]
        EPA_MASTER -.disabled.-> FILTER
        CURATED --> TAXONOMY
        FILTER --> TAXONOMY --> SCOPED_JSON
    end
